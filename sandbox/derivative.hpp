...
public:
  ...
  NumericalDerivative(const function_type& f_, scalar_type ep = 0.):
    Functor_(f_){defaultEpsilon(ep);}

  NumericalDerivative(scalar_type ep = 0.):function_type(){defaultEpsilon(ep);}
  
  template<typename T0>
  NumericalDerivative(const T0& t0):function_type(t0) {defaultEpsilon();}

  template<typename T0, typename T1>
  NumericalDerivative(const T0& t0, const T1& t1):function_type(t0, t1) {defaultEpsilon();}

  template<typename T0, typename T1, typename T2>
  NumericalDerivative(const T0& t0, const T1& t1, const T2& t2)
    :function_type(t0, t1, t2) {defaultEpsilon();}

  template<typename T0, typename T1, typename T2, typename T3>
  NumericalDerivative(const T0& t0, const T1& t1, const T2& t2, const T3& t3)
    :function_type(t0, t1, t2,t3) {defaultEpsilon();}
